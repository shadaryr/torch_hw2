...
local f = assert(io.open('logFile_adamax.log', 'w'), 'Failed to open input file')
 --print('open the file')
...
local f = assert(io.open('logFile_adamax.log', 'a+'), 'Failed to open input file')
        if e > 1 then
        ...
        if (testError[e] < bestError) then
            ...
            torch.save('HW2_network_adamax.t7', model)
...
require 'gnuplot'
local range = torch.range(1, epochs)
gnuplot.pngfigure('loss_adamax.png')
gnuplot.plot({'trainLoss',trainLoss},{'testLoss',testLoss})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Loss')
gnuplot.plotflush()

gnuplot.pngfigure('error_adamax.png')
gnuplot.plot({'trainError',trainError},{'testError',testError})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Error')
