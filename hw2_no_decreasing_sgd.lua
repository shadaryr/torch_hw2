...
local f = assert(io.open('logFile_sgd_constant_learning_rate_no_batchfilp.log', 'w'), 'Failed to open input file')
 --print('open the file')
   --f:write('The model is: ')
--print('start print to the log')
   --f:write(model)
   f:write('Number of parameters: ')
   f:write('Description of model: sgd, NO batchflip, dropout 0.2, learning rate constant')
   f:write(w:nElement())
...
local f = assert(io.open('logFile_sgd_constant_learning_rate_no_batchfilp.log', 'a+'), 'Failed to open input file')
        if e > 1 then
        ...
        if (testError[e] < bestError) then
            ...
            torch.save('HW2_network_sgd_constant_learning_rate_no_batchfilp.t7', model)
...
require 'gnuplot'
local range = torch.range(1, epochs)
gnuplot.pngfigure('loss_sgd_constant_learning_rate_no_batchfilp.png')
gnuplot.plot({'trainLoss',trainLoss},{'testLoss',testLoss})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Loss')
gnuplot.plotflush()

gnuplot.pngfigure('error_sgd_constant_learning_rate_no_batchfilp.png')
gnuplot.plot({'trainError',trainError},{'testError',testError})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Error')
