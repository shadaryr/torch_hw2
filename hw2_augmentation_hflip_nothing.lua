  ...
  function BatchFlip:updateOutput(input)
    if self.train then
      ...
      for i=1,input:size(1) do
        if 0 == permutation[i] % 2  then image.hflip(input[i]) end
...
local f = assert(io.open('logFile_sgd_decreasing_learning_rate_batchfilp_2_hflip_nothing.log', 'w'), 'Failed to open input file')
 --print('open the file')
   --f:write('The model is: ')
--print('start print to the log')
   --f:write(model)
   f:write('Number of parameters: ')
   f:write('Description of model: sgd, batchflip: 1/2 hflip 1/2 nothing, dropout 0.2, learning rate decrease by 2 factor every 5 epochs')
   f:write(w:nElement())
...
local f = assert(io.open('logFile_sgd_decreasing_learning_rate_batchfilp_2_hflip_nothing.log', 'a+'), 'Failed to open input file')
        if e > 1 then
        ...
        if (testError[e] < bestError) then
            ...
            torch.save('HW2_network_sgd_decreasing_learning_rate_batchfilp_2_hflip_nothing.t7', model)
...
require 'gnuplot'
local range = torch.range(1, epochs)
gnuplot.pngfigure('loss_sgd_decreasing_learning_rate_batchfilp_2_hflip_nothing.png')
gnuplot.plot({'trainLoss',trainLoss},{'testLoss',testLoss})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Loss')
gnuplot.plotflush()

gnuplot.pngfigure('error_sgd_decreasing_learning_rate_batchfilp_2_hflip_nothing.png')
gnuplot.plot({'trainError',trainError},{'testError',testError})
gnuplot.xlabel('epochs')
gnuplot.ylabel('Error')
