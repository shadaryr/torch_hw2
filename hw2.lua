...
model:add(cudnn.SpatialConvolution(64, 32, 3, 3)) --gets 6*6*64. 10 filters. 3*3 is the surface of each kernel floor((6+2*0-3)/1 +1)*floor((6+2*0-3)/1 +1)*32=4*4*32
--model:add(cudnn.SpatialAveragePooling(4,4,4,4)) --gets 4*4*10, returns 1*1*10
model:add(nn.View(32*4*4):setNumInputDims(3))  -- reshapes from a 3D tensor of 32x1x1 into 1D tensor of 32*4*4
