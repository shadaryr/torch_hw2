...
model:add(cudnn.SpatialConvolution(32, 64, 5, 5)) -- gets 14*14*32. 64 filters. 5*5 is the surface of each kernel floor((14+2*0-5)/1 +1)*floor((14+2*0-5)/1 +1)*64=10*10*64
...
model:add(nn.View(32*3*3):setNumInputDims(3))  -- reshapes from a 3D tensor of 32x4x4 into 1D tensor of 32*3*3
model:add(nn.Linear(32*3*3, 64))             -- fully connected layer (matrix multiplication between input and weights). gets a 32*3*3 vector, outputs 64 neurons. parameters (32*3*3+1)*64 (the +1 is bias)
model:add(cudnn.ReLU(true))
