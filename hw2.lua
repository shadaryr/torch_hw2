...
model:add(cudnn.SpatialConvolution(3, 64, 5, 5)) -- 3 input image channel, 64 output channels, 5x5 convolution kernel. owidth=floor((32+2*0-5)/1 +1)=28. same goes to ohight. output- 28*28*64
model:add(cudnn.SpatialMaxPooling(2,2,2,2))      -- A max-pooling operation that looks at 2x2 windows and finds the max. floor(28+2*0-2)/2+1)*floor(28+2*0-2)/2+1)*64(depth do not change) = 14*14*64
...
model:add(cudnn.SpatialConvolution(64, 16, 3, 3)) -- gets 14*14*64. 16 filters. 3*3 is the surface of each kernel floor((14+2*0-3)/1 +1)*floor((14+2*0-3)/1 +1)*16=12*12*16
...
model:add(cudnn.SpatialConvolution(16, 16, 3, 3)) --gets 6*6*16. 16 filters. 3*3 is the surface of each kernel floor((6+2*0-3)/1 +1)*floor((6+2*0-3)/1 +1)*16=4*4*16
